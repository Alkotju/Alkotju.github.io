(() => {
    ...
    const feedbacklEl = document.getElementById("feedback");
    ...
    const showFeedback = (text, isError = false) => {
        ...
        feedbacklEl.classList.toggle("feedback--error", isError);
    };

    //Получаем историю через REST, чтобы сраззу показать уже отправленные сообщения
