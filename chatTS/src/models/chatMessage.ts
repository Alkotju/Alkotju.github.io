export interface ChatMessage {
    //Уникальный идетификатор, чтобы удобно рендерить список и отлаживать историю
    id: number;
...
export class ChatModel {
    ...
    //Счетчик для генерации уникаьлных идетификаторов
    private nextId = 1;
    // Ограничение на длину истории, чтобы не раздувать память
    private readonly maxMessages = 100;

    //Вернуть копии массива сообщений, чтобы снаружи нельзя было мутировать состояние
    public getAll(): ChatMessage[]{
        return [...this.messages];
    }

    // Добавить новое сообщение: возвращаем итоговый объект для дальнейщей рассылки клиентам
    public add(author: string, text: string): ChatMessage {
        const trimmedAuthor = author.trim() || "Anonymous";
        const trimmedText = text.trim();

        //Пустые строки игнорируем, чтобюы не захламлять чат
        if (!trimmedText) {
